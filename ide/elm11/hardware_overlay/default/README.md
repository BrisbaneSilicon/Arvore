# Default hardware overlay

Place the default overlay image here as **`emblua.vg`**.

When a user creates a new Lua workspace **without** ticking *Custom Hardware
Overlay*, the IDE copies this `emblua.vg` into the workspace as
`hardware/.build/emblua.vg` so the project can be synthesised straight away
(see `HardwareOverlayPanel.deploy_default_overlay`).
