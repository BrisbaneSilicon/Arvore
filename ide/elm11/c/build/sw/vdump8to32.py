...
if len(sys.argv) < 6 or '-h' in sys.argv:
    print("Usage: python vdump8to32.py <input 8b verilog hex path> <output path> <v32|vx4|mif|mi|mix4> <mem addr base> <mem size kb>")
    sys.exit()
...
if foutfmt == "v32":
    ...
    print("@00000000", file=fout)
    for u in membuf:
        u_le = ((u & 0xFF) << 24) | ((u & 0xFF00) << 8) | ((u & 0xFF0000) >> 8) | ((u & 0xFF000000) >> 24)
        print("{:08X}".format(u_le), file=fout)
    print("", file=fout)
    fout.close()
elif foutfmt == "vx4":
