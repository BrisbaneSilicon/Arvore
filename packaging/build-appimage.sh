#!/usr/bin/env bash
# Build an AppImage for the ELM11 IDE.
#
# Prerequisites:
...
ICON_SRC="$ROOT/elm11_ide/elm11-ide.png"
if [[ ! -f "$ICON_SRC" ]]; then
...
cp -r "$DIST/elm11-ide/." "$APPDIR/usr/bin/"

# Desktop entry + icon (must live at both AppDir root and the standard path)
cp "$HERE/elm11-ide.desktop" "$APPDIR/elm11-ide.desktop"
cp "$HERE/elm11-ide.desktop" "$APPDIR/usr/share/applications/elm11-ide.desktop"
cp "$ICON_SRC" "$APPDIR/elm11-ide.png"
cp "$ICON_SRC" "$APPDIR/usr/share/icons/hicolor/256x256/apps/elm11-ide.png"
...
cp -f "$HERE/elm11-ide.png" "$ICON_DIR/elm11-ide.png" 2>/dev/null || true
...
cat > "$APPS_DIR/elm11-ide.desktop" <<DESK
[Desktop Entry]
Type=Application
Name=ELM11 IDE
...
ARCH="${ARCH:-$(uname -m)}"
export ARCH
OUT="$DIST/ELM11_IDE-${ARCH}.AppImage"
rm -f "$OUT"
